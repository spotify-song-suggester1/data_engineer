...
def add_song(song_to_predict):
    ...
    exists = bool(Song.query.filter_by(id=song_id).first())
    if not exists:
        DB.session.add(Song(id= song_id, name =name,energy =energy,liveness =liveness,danceability =danceability,instrumentalness =instrumentalness,loudness =loudness,speechiness =speechiness,valence =valence,tempo =tempo))
        DB.session.commit()
